function [GMLMstructure, trials, spkHistPrior_setup, prior_setup, trialsUsed, couplingShuffle, normalizationSettings, timeWindows] = constructGMLMdata13b(data, modelSetup, varargin)
...
if(~isfield(modelSetup, "baselineCorrectSpikeHistory"))
    modelSetup.baselineCorrectSpikeHistory = false;
end
if(~isfield(modelSetup, "baselineCorrectLocalCoupling"))
    modelSetup.baselineCorrectLocalCoupling = modelSetup.baselineCorrectSpikeHistory;
end
...
if(modelSetup.includeCouplingLocal)
    % local coupling
    jj = jj + 1;
    if((modelSetup.baselineCorrectSpikeHistory == modelSetup.baselineCorrectLocalCoupling) || modelSetup.baselineCorrectionType == "none")
    ...
    GMLMstructure.Groups(jj).X_shared = {modelSetup.bases.spkCoupling.B};
    ...
    GMLMstructure.Groups(jj).factor_is_spk_coupling = true;
...
for tt = 1:numel(data.trials)
    ...
    if(modelSetup.baselineCorrectionType ~= "none" && modelSetup.baselineCorrectSpikeHistory)
        ...
        Y_t = correctSpikeHistory(Y_t, Y_b, modelSetup.baselineCorrectionType);
    end
...
%         Y_t(:) = 0;
%         Y_t(t_0 + 2,:) = 1;
        if(modelSetup.baselineCorrectionType ~= "none" && (isempty(localCouplingGroup) || jj ~= localCouplingGroup || modelSetup.baselineCorrectSpikeHistory))
            error("Cannot correct spike history with this setup!");
...
%             Y_t = correctSpikeHistory(Y_t, Y_b, modelSetup.baselineCorrectionType);
        ...
        [rr,neuronNums] = find(Y_t);
        rr = rr - t_0 + 1;
        trials(tt).Groups(jj).spkTimes  = cell(dim_P_c,1); 
        for pp = 1:dim_P_c
            trials(tt).Groups(jj).spkTimes{pp} = sort(rr(neuronNums == pp));
        end
...
if(~isfield(modelSetup, "normalizationSettings") && false)
...
%         end
    ...
    for jj = 1:numel(GMLMstructure.Groups)
        ...
        if(~GMLMstructure.Groups(jj).is_coupling)
            ...
            if(~isempty(localCouplingGroup) && jj == localCouplingGroup)
...
warning("TOOK OUT NORMALIZATION");
