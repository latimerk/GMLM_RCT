function [GMLMstructure, trials, spkHistPrior_setup, prior_setup, trialsUsed, couplingShuffle, normalizationSettings, timeWindows] = constructGMLMdata13(data, modelSetup, varargin)
...
paddedCoupling = [zeros(size(modelSetup.bases.spkHist.B) + [1 0]); modelSetup.bases.spkHist.B];
...
for tt = 1:numel(data.trials)
    ...
    for cc = 1:numel(coupling_locations)
        ...
        trials(tt).Groups(jj).X_local{1} = zeros([TT size(modelSetup.bases.spkHist.B,2) dim_P_c]);  
