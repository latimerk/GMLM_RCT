function [] = plotElpdDiff2(CC2, CC1, fl, sg, NSEs)
...
if(nargin < 2 || isempty(CC1))
    ...
    CC1.filterLength = CC2(1).filterLength;
end
...
colors = [ 0    0.4470    0.7410;
    ...
    0.9290    0.6940    0.1250];
NM = numel(CC2);
dd = zeros([size(CC1.X2) NM]);


if(nargin > 2 && ~isempty(fl) && fl == 1)
    for mm = 1:NM
        dd(:,:,:,mm) =  sg*(CC2(mm).X2f - CC1.X2f);

        if(CC1.filterLength ~= CC2(mm).filterLength)
            error("Filter lengths are different");
        end
    end
    fl = CC1.filterLength;
elseif(nargin > 2 && ~isempty(fl) && fl == 2)
    for mm = 1:NM
        dd(:,:,:,mm) =  sg*(CC2(mm).FR - CC1.FR);
