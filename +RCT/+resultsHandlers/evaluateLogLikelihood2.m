function [LL_s, dim_N_ranges, LL, Z] = evaluateLogLikelihood2(gmlm, samples, modelSetup)
...
for sample_idx = 1:NS
    ...
    if(~modelSetup.includeTrialwiseLatent)
        ...
        %% 
        LL(:,sample_idx) = sum(LL_info.logLikeFun(log_rate, Y, dt),2);
    else
        ...
        LL(:,sample_idx) = kgmlm.utils.logMeanExp(llc,2);
    ...
    for mm = 1:dim_M
        ...
        Z(mm, sample_idx) = sum(LL(xx,sample_idx),1);
    end
end


LL_s = kgmlm.utils.logMeanExp(LL,2);
