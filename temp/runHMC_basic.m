function [samples,M] = runHMC_basic(HMC_settings, yy)
if(nargin < 2)
    yy = 0.1;
end
figNum = 1;
...
for sample_idx = 2:TotalSamples
    ...
    [samples.accepted(sample_idx), samples.errors(sample_idx), theta_new, samples.log_p_accept(sample_idx)] = kgmlm.fittingTools.HMCstep_diag(theta, M, nlpostFunction, HMC_state);
    theta = theta_new;
